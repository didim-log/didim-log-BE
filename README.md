...
| Gemini 공급자 호출 제한 | 실제 Redis 7.2.5, 최소 간격 키 없이 같은 시각에 20건 요청 | 허용 15건·RPM 20·RPD 15 → 허용 1건·RPM 1·RPD 1 |
...
| 회고 저장소 원자 연산 | 실제 MongoDB 7.0.16·lifecycle 잠금 제외, 북마크 2건 동시 토글·최초 작성 2건·수정 중 선행 삭제 | 토글 2회 모두 반영, 회고 1건 저장, 삭제 뒤 기존 ID 미복원 |
...
| 문제 수집 만료 작업 인계 | 실제 Redis 7.2.5의 `PENDING` 재제출·만료 `RUNNING` 경합·4개 작업 유형 manifest suffix·실행기 거부·손상 manifest | claim 성공 1건, 외부 호출·저장 각 1건, prefix 재호출 0건, 기존 카운터 보존, 삭제 대상 실패 기록, 유효 lease 갱신 0건 |
