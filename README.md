...
- lease가 없는 `PENDING`·`RUNNING` 작업은 scanner가 다시 제출하고, 현재 `processedCount` 이후 manifest 대상부터 이어서 실행합니다.
...
sequenceDiagram
    ...
    API->>Redis: PENDING 상태·대상 manifest·작업 index 원자 저장
    API-)Worker: 수집 작업 제출
    API-->>Admin: jobId
    ...
    Worker->>Redis: CAS로 진행률·checkpoint·실패 ID 갱신
    Admin->>API: 작업 상태 조회
...
- **[AI 리뷰 중복 생성](./DOCS/performance/runs/2026-06-21-DIDIMLOG-LOCAL-VERIFICATION.md)**
  - 조건: 동일 로그 50건 동시 요청·10회 반복
  - 결과: 각 회차 Gemini 호출 1회·저장 1건, 최종 `COMPLETED`, 잠금 잔존 0건

- **[AI 리뷰 잠금 소유권](./DOCS/refactoring/be-refactor/PHASE_6C_AI_REVIEW_LOCK_OWNERSHIP.md)**
  - 조건: 실제 MongoDB 7.0.16, A lease 만료·B 재획득·A 완료/실패
  - 결과: 만료 직후와 B 재획득 뒤 A 갱신 0건, B 상태 보존·종료 성공

- **[AI 일일 사용량](./DOCS/refactoring/be-refactor/PHASE_5A_AI_USAGE_ATOMIC_RESERVATION.md)**
  - 조건: 실제 Redis, 남은 사용자·전역 한도 각각 1회에서 20건 동시 예약·동일 예약 2회 반환
  - 결과: 각 경계 성공 1건·거절 19건, 카운터 한도 유지, 동일 예약은 1회만 감소

- **[Gemini 공급자 호출 제한](./DOCS/refactoring/be-refactor/PHASE_6D_GEMINI_RATE_LIMIT_ATOMICITY.md)**
  - 조건: 실제 Redis 7.2.5, 최소 간격 키 없이 같은 시각에 20건 요청
  - 결과: 허용 15건·RPM 20·RPD 15 → 허용 1건·RPM 1·RPD 1

- **[인증 요청 제한](./DOCS/refactoring/be-refactor/PHASE_2D_AUTH_RATE_LIMIT_ATOMICITY.md)**
  - 조건: 실제 Redis 동일 키 20건 동시 요청·한도 1건
  - 결과: 허용 1건·차단 19건, 최종 카운터 1

- **[Refresh Token 회전](./DOCS/refactoring/be-refactor/PHASE_2B_REFRESH_TOKEN_ATOMIC_ROTATION.md)**
  - 조건: 실제 Redis 7.2.5, 같은 기존 토큰으로 20건 동시 교체
  - 결과: 성공 1건, 기존 토큰 0건, 새 토큰 1건

- **[OAuth 로그인 코드 교환](./DOCS/refactoring/be-refactor/PHASE_2C_4_OAUTH_CODE_EXCHANGE.md)**
  - 조건: 실제 Redis, 같은 일회용 코드를 동시에 소비
  - 결과: 성공 1건, 이후 요청은 같은 만료·재사용 오류

- **[JWT 토큰 용도 분리](./DOCS/refactoring/be-refactor/PHASE_2C_1_JWT_TOKEN_PURPOSE.md)**
  - 조건: 실제 Security Filter Chain에 Access·Refresh Token 전달
  - 결과: Access Token 인증 성공, Refresh Token의 보호 API 접근 401

- **비밀번호 재설정 코드 발급·소비**
  - 조건: 실제 MongoDB 7.0.16, 같은 회원 20건 동시 발급·같은 코드 20건 동시 재설정
  - 결과: 발급 오류 0건·활성 코드 1건, 재설정 성공 1건·실패 19건
  - 문서: [발급 정합성](./DOCS/refactoring/be-refactor/PHASE_2B_3_PASSWORD_RESET_ISSUANCE_CONSISTENCY.md) · [원자적 소비](./DOCS/refactoring/be-refactor/PHASE_2B_2_PASSWORD_RESET_ATOMIC_CONSUME.md)

- **[비밀번호 변경과 토큰 소유자 고정](./DOCS/refactoring/be-refactor/PHASE_2E_PASSWORD_CHANGE_SESSION_REVOCATION.md)**
  - 조건: 실제 MongoDB 7.0.16·Redis 7.2.5, 기존 Access/Refresh Token·지연 로그인·BOJ ID 재사용·과거 Student 저장
  - 결과: 구 버전·다른 학생 토큰 거절, stale CAS·전체 저장 무변경

- **[로그인 프로필 동기화](./DOCS/refactoring/be-refactor/PHASE_2C_2_LOGIN_PROFILE_PARTIAL_UPDATE.md)**
  - 조건: 실제 MongoDB, solved.ac 조회 대기 중 비밀번호 재설정
  - 결과: 새 비밀번호 유지, rating·tier 필드만 갱신

#### 사용자·학습 데이터

- **[가입·회고 DB 정합성](./DOCS/refactoring/be-refactor/PHASE_2A_DATA_CONSISTENCY.md)**
  - 조건: 실제 MongoDB, 같은 학생·문제 회고 2건 동시 insert
  - 결과: 학생 식별자 중복 거부, 회고 1건 저장, 기존 중복 데이터 자동 삭제 0건

- **[풀이 결과 동시 저장](./DOCS/refactoring/be-refactor/PHASE_3A_STUDY_SOLUTION_CAS.md)**
  - 조건: 실제 MongoDB 7.0.16, 같은 문서 버전의 다른 문제 2건·같은 문제 2건·비밀번호 변경 경합
  - 결과: 다른 문제 2건 모두 보존, 같은 문제 당일 1건, 새 자격 증명 유지

- **[회고 저장소 원자 연산](./DOCS/refactoring/be-refactor/PHASE_3B_RETROSPECTIVE_ATOMIC_UPDATES.md)**
  - 조건: 실제 MongoDB 7.0.16·lifecycle 잠금 제외, 북마크 2건 동시 토글·최초 작성 2건·수정 중 선행 삭제
  - 결과: 토글 2회 모두 반영, 회고 1건 저장, 삭제 뒤 기존 ID 미복원

- **[계정 삭제 경로 통합](./DOCS/refactoring/be-refactor/PHASE_3C_ACCOUNT_DELETION_CONSISTENCY.md)**
  - 조건: 실제 MongoDB 7.0.16, 본인 탈퇴·관리자 삭제와 사용자 소유 5개 컬렉션
  - 결과: 두 경로 모두 같은 범위를 삭제하고 시스템·다른 사용자 데이터 보존

- **[계정 삭제와 쓰기 경합](./DOCS/refactoring/be-refactor/PHASE_3D_ACCOUNT_WRITE_BARRIER.md)**
  - 조건: 실제 MongoDB 7.0.16·Redis 7.2.5, 템플릿 생성 선행·계정 삭제 선행 신규 회고
  - 결과: 충돌 요청 409, 재시도 삭제 뒤 Student·사용자 문서 0건

- **[기본 템플릿 삭제 정합성](./DOCS/refactoring/be-refactor/PHASE_3E_TEMPLATE_REFERENCE_CONSISTENCY.md)**
  - 조건: 실제 MongoDB 7.0.16·Redis 7.2.5, 양쪽 기본값·삭제 실패·설정/삭제 경합
  - 결과: 참조 선행 해제, SYSTEM 참조 보존, 충돌 409, 재시도 뒤 깨진 참조 0건

#### 문제 수집

- **[문제 상세·메타데이터 저장 경합](./DOCS/refactoring/be-refactor/PHASE_5B_CRAWLER_DETAIL_PARTIAL_UPDATE.md)**
  - 조건: 실제 MongoDB 7.0.16, 상세 대상 조회 뒤 메타데이터·언어 갱신 후 상세 저장
  - 결과: 최신 메타데이터·언어 보존, 오래된 전체 문서 덮어쓰기 1건 → 0건, 삭제 대상 재생성 0건

- **[문제 수집 작업 상태](./DOCS/refactoring/be-refactor/PHASE_6E_CRAWLER_JOB_STATE_CAS.md)**
  - 조건: 동일 Runnable 재실행, 실제 Redis 7.2.5의 두 서비스 상태 경합, 문제 6건 수집 명령 계측
  - 결과: 외부 호출 2건 → 1건, 취소 성공 2건 → 1건, 취소 뒤 최종 상태 `COMPLETED` → `CANCELLED`, 작업당 index `ZADD` 9회 → 1회

- **[문제 수집 재시작 고아 작업](./DOCS/refactoring/be-refactor/PHASE_6K_CRAWLER_STARTUP_ORPHAN_RECOVERY.md)**
  - 조건: 실제 Redis 7.2.5, 진행 작업 2건·종료 작업 3건·두 서비스 12건 CAS 경합·실행 중 worker
  - 결과: 진행 작업만 `FAILED`, 종료 상태 원문 유지, 작업별 성공 전이 1회, 다음 문제 호출·완료 덮어쓰기 0건

- **[문제 수집 실패 항목 재시도](./DOCS/refactoring/be-refactor/PHASE_6H_CRAWLER_FAILED_ITEM_RETRY.md)**
  - 조건: 실제 Redis 7.2.5, 1~5번 중 문제 ID 2만 실패·취소 뒤 늦은 실패 상태 저장
  - 결과: 재시도 시 2번 호출 0건 → 1건, 미복구 문제 1건 → 0건, 기존 성공 문제 재호출 0건

- **[문제 수집 재시도 대상 manifest](./DOCS/refactoring/be-refactor/PHASE_6L_CRAWLER_TARGET_MANIFEST.md)**
  - 조건: 실제 Redis 7.2.5의 비연속 `[2, 5]`·혼합 `explicitIds + range`·손상 상태, MockK 비메타데이터 조회
  - 결과: 저장 대상 밖 호출 0건, 현재 DB 전체 조회 0건, 손상·처리 위치 불일치 자식 작업 0건

- **[문제 수집 worker lease](./DOCS/refactoring/be-refactor/PHASE_6M_A_CRAWLER_WORKER_LEASE.md)**
  - 조건: 실제 Redis 7.2.5의 동시 claim·heartbeat/진행률 경합·lease 교체·취소, MockK 외부 클라이언트·저장소
  - 결과: claim 1건, MockK 외부 클라이언트·저장소 호출 각 1건, stale 상태·실패 원장 쓰기 0건, 종료 뒤 lease 0건

- **[문제 수집 만료 작업 인계](./DOCS/refactoring/be-refactor/PHASE_6M_B_CRAWLER_WORKER_TAKEOVER.md)**
...
- **[고아 데이터 읽기 전용 점검](./DOCS/refactoring/be-refactor/PHASE_5C_ORPHAN_DATA_READ_ONLY_DRY_RUN.md)**
  - 조건: 실제 MongoDB 7.0.16 합성 fixture, 사용자 소유 관계 5개·기본 참조 2개
  - 결과: fixture 기대 고아 6건·깨진 기본 참조 2건 일치, 쓰기 단계 0건·점검 필드 전후 hash 동일

- **[유지보수 설정 원자화](./DOCS/refactoring/be-refactor/PHASE_6B_MAINTENANCE_CONFIG_ATOMICITY.md)**
  - 조건: 실제 Redis 7.2.5, 종료 시각 만료·유한→무기한 변경·두 서비스 동시 설정
  - 결과: 설정 키 전체 만료, 무기한 설정 TTL 없음, 서로 다른 설정 조합 0건

위 항목은 로컬 MongoDB·Redis와 Gemini Mock을 사용한 정확성 검증이며 운영 성능을 뜻하지 않습니다.

상세 구현과 검증 근거는 각 항목의 제목에 연결했습니다.
...
- worker lease 모드는 10초마다 lease가 없는 작업을 실행기에 다시 제출합니다.
  claim은 Runnable이 시작된 뒤에만 수행하므로 큐 대기 중에는 lease가 생기지
  않으며, 실행기 거부 시 다음 scan에서 다시 제출합니다.
...
`PROBLEM_COLLECTOR_WORKER_LEASE_ENABLED`도 기본값은 `false`입니다. Redis 상태
소유권 검증과 만료 작업 자동 인계는 포함하지만 이미 시작한 MongoDB 쓰기를
차단하지 않으므로, 같은 항목의 중복 쓰기를 허용할 수 있는 환경에서만
활성화합니다.
