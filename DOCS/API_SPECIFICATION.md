...
| DELETE | `/api/v1/students/me` | 로그인한 사용자의 Refresh Token, 재설정 코드, 회고, 피드백, 커스텀 템플릿, 학생 ID가 있는 로그와 계정을 삭제합니다. 시스템 템플릿과 소유자를 확정할 수 없는 이전 로그는 보존합니다. 삭제 작업 충돌 시 `409`, 세션 상태 저장소 장애 시 `503`을 반환합니다. (Hard Delete, 복구 불가) | **Headers:**<br>- `Authorization: Bearer {token}` (required): JWT 토큰 | `204 No Content` (성공 시) | JWT Token |
...
| DELETE | `/api/v1/admin/users/{studentId}` | 본인 탈퇴와 같은 공통 삭제 흐름으로 특정 회원의 Refresh Token, 재설정 코드, 회고, 피드백, 커스텀 템플릿, 학생 ID가 있는 로그와 계정을 삭제합니다. 성공 뒤 `USER_DELETE` 감사 이력을 남깁니다. 대상 없음은 `404`, 작업 충돌은 `409`, 세션 상태 저장소 장애는 `503`입니다. | **Headers:**<br>- `Authorization: Bearer {token}` (required): JWT 토큰 (ADMIN role 필요)<br><br>**Path Variables:**<br>- `studentId` (String, required): 학생 ID | `Map<String, String>`<br><br>**응답 구조:**<br>- `message` (String): 응답 메시지 ("회원이 성공적으로 탈퇴되었습니다.") | JWT Token (ADMIN) |
...
| GET | `/api/v1/admin/feedbacks` | 페이징을 적용하여 피드백 목록을 조회합니다. ADMIN 권한이 필요합니다. | **Headers:**<br>- `Authorization: Bearer {token}` (required): JWT 토큰 (ADMIN role 필요)<br><br>**Query Parameters:**<br>- `page` (Int, optional, default: 1): 페이지 번호 (1부터 시작)<br>  - 유효성: `@Min(1)` (1 이상)<br>- `size` (Int, optional, default: 20): 페이지 크기<br>  - 유효성: `@Positive` (1 이상) | `Page<FeedbackResponse>`<br><br>**FeedbackResponse 구조:**<br>- `id` (String): 피드백 ID<br>- `writerId` (String): 작성자 ID (Student ID)<br>- `bojId` (String, nullable): 연결된 BOJ ID<br>- `content` (String): 피드백 내용<br>- `type` (String): 피드백 유형 ("BUG", "SUGGESTION")<br>- `status` (String): 처리 상태 ("PENDING", "COMPLETED")<br>- `createdAt` (LocalDateTime): 생성 일시<br>- `updatedAt` (LocalDateTime): 수정 일시<br><br>**Page 구조:**<br>(위와 동일) | JWT Token (ADMIN) |
...
`FeedbackResponse.bojId`는 연결된 BOJ ID이며 nullable입니다. BOJ 연동을 하지
않았거나 작성자 계정이 삭제된 피드백은 `null`로 반환합니다.
...
**예시 요청 (피드백 목록 조회):**
```http
GET /api/v1/admin/feedbacks?page=1&size=20
Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
```
...
| Method | URI | 기능 설명 | Request | Response | Auth |
|--------|-----|----------|---------|----------|------|
| GET | `/api/v1/admin/dashboard/stats` | 총 회원 수, 오늘 가입한 회원 수, 총 해결된 문제 수, 오늘 작성된 회고 수를 조회합니다. ADMIN 권한이 필요합니다. | **Headers:**<br>- `Authorization: Bearer {token}` (required): JWT 토큰 (ADMIN role 필요) | `AdminDashboardStatsResponse`<br><br>**AdminDashboardStatsResponse 구조:**<br>- `totalUsers` (Long): 총 회원 수<br>- `todaySignups` (Long): 오늘 가입한 회원 수<br>- `totalSolvedProblems` (Long): 전체 회원의 SUCCESS 풀이 중 고유 문제 ID 수<br>- `todayRetrospectives` (Long): 오늘 작성된 회고 수 | JWT Token (ADMIN) |
| GET | `/api/v1/admin/dashboard/chart` | 회원·해결 문제·회고의 누적 추이를 일·ISO 주차·월 단위로 조회합니다. 해결 문제는 문제별 최초 성공 시점에 한 번만 반영합니다. | **Headers:**<br>- `Authorization: Bearer {token}` (required): JWT 토큰 (ADMIN role 필요)<br>**Query:**<br>- `dataType` (required): `USER`, `SOLUTION`, `RETROSPECTIVE`<br>- `period` (required): `DAILY`, `WEEKLY`, `MONTHLY` | `ChartDataResponse`<br><br>**data 항목:**<br>- `date` (String): `yyyy-MM-dd`, `YYYY-Www` 또는 `yyyy-MM` 형식의 구간<br>- `value` (Long): 해당 구간까지의 누적값 | JWT Token (ADMIN) |
...
| Method | URI | 기능 설명 | Request | Response | Auth |
|--------|-----|----------|---------|----------|------|
| POST | `/api/v1/feedback` | 버그 리포트 또는 건의사항을 등록합니다. JWT 토큰에서 사용자 ID를 자동으로 추출합니다. | **Headers:**<br>- `Authorization: Bearer {token}` (required): JWT 토큰<br><br>**Request Body:**<br>`FeedbackCreateRequest`<br>- `content` (String, required): 피드백 내용<br>  - 유효성: `@NotBlank`, 최소 10자 이상<br>- `type` (FeedbackType, required): 피드백 유형<br>  - 값: "BUG" (버그 리포트), "SUGGESTION" (건의사항) | `FeedbackResponse`<br><br>**FeedbackResponse 구조:**<br>- `id` (String): 피드백 ID<br>- `writerId` (String): 작성자 ID (Student ID)<br>- `bojId` (String, nullable): 연결된 BOJ ID<br>- `content` (String): 피드백 내용<br>- `type` (String): 피드백 유형 ("BUG", "SUGGESTION")<br>- `status` (String): 처리 상태 ("PENDING", "COMPLETED")<br>- `createdAt` (LocalDateTime): 생성 일시<br>- `updatedAt` (LocalDateTime): 수정 일시 | JWT Token |

`FeedbackResponse.bojId`는 연결된 BOJ ID이며, BOJ 연동이 없으면 `null`입니다.
...
| Method | URI | 기능 설명 | Request | Response | Auth |
|--------|-----|----------|---------|----------|------|
| GET | `/api/v1/templates` | 인증된 사용자의 커스텀 템플릿과 시스템 기본 템플릿 목록을 조회합니다. 저장된 기본값 ID가 없거나 현재 목록에 포함되지 않으면 시스템 fallback 기준으로 `isDefaultSuccess`/`isDefaultFail`을 계산합니다. JWT 토큰에서 사용자 정보를 자동으로 추출합니다. | **Headers:**<br>- `Authorization: Bearer {token}` (required): JWT 토큰 | `List<TemplateResponse>`<br><br>**TemplateResponse 구조:**<br>- `id` (String): 템플릿 ID<br>- `studentId` (String, nullable): 템플릿 소유자 ID (시스템 템플릿은 null)<br>- `title` (String): 템플릿 이름<br>- `content` (String): 템플릿 내용 (마크다운 원본)<br>- `type` (String): 템플릿 타입 ("SYSTEM", "CUSTOM")<br>- `isDefaultSuccess` (Boolean): 성공용 기본 템플릿 여부<br>- `isDefaultFail` (Boolean): 실패용 기본 템플릿 여부<br>- `createdAt` (LocalDateTime): 생성 일시<br>- `updatedAt` (LocalDateTime): 수정 일시 | JWT Token |
...
| DELETE | `/api/v1/templates/{id}` | 커스텀 템플릿을 삭제합니다. 성공·실패 기본값으로 사용 중이면 Student의 일치하는 참조를 먼저 해제하고 이후 시스템 기본 템플릿을 사용합니다. 시스템 템플릿은 삭제할 수 없습니다. JWT 토큰에서 사용자 정보를 자동으로 추출합니다. | **Headers:**<br>- `Authorization: Bearer {token}` (required): JWT 토큰<br><br>**Path Variables:**<br>- `id` (String, required): 템플릿 ID | `204 No Content` (응답 본문 없음) | JWT Token |
