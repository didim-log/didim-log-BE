...
| Method | URI | 기능 설명 | Request | Response | Auth |
|--------|-----|----------|---------|----------|------|
| GET | `/api/v1/problems/recommend` | 학생의 현재 티어를 기반으로 적절한 난이도의 문제를 추천합니다. **추천 로직:** Solved.ac의 Rating(점수)을 Source of Truth로 사용하여 티어를 판정하고, 해당 티어 레벨 범위에서 -2 ~ +2 단계의 난이도 문제 중 아직 풀지 않은 문제를 추천합니다. **UNRATED 사용자 특별 처리:** UNRATED 티어(레벨 0, rating=0) 사용자는 Bronze V(레벨 1) ~ Bronze IV(레벨 2) 문제를 추천받습니다. 카테고리를 지정하면 해당 카테고리 문제만 추천합니다. JWT 토큰에서 사용자 정보를 자동으로 추출합니다. **태그 별칭 지원:** 축약형 태그(예: "BFS", "DFS", "DP")를 입력하면 자동으로 공식 전체 이름(예: "Breadth-first Search")으로 변환하여 검색합니다. **카테고리 확장 검색:** 상위 카테고리를 선택하면 하위 태그들도 자동으로 포함하여 검색합니다. 예: "Graph Theory"를 선택하면 "BFS", "DFS", "Dijkstra", "MST", "LCA" 등 하위 태그가 포함된 문제도 함께 검색됩니다. | **Headers:**<br>- `Authorization: Bearer {token}` (required): JWT 토큰<br><br>**Query Parameters:**<br>- `count` (Int, optional, default: 10): 추천할 문제 개수<br>  - 유효성: `@Min(1)` (최소 1개 이상), `@Max(50)` (최대 50개 이하)<br>  - 범위를 벗어나면 400 Bad Request<br>- `category` (String, optional): 문제 카테고리 필터<br>  - **지원 형식:**<br>    - 축약형 태그: "BFS", "DFS", "DP", "MST", "LCA", "KMP", "FFT", "LIS", "LCS" 등<br>    - 공식 전체 이름: "Breadth-first Search", "Depth-first Search", "Dynamic Programming" 등<br>    - Enum 이름: "IMPLEMENTATION", "GRAPH_THEORY", "BFS", "DFS", "DP" 등<br>    - 한글 이름: "그래프 이론", "다이나믹 프로그래밍" 등<br>  - **자동 변환:** 축약형 태그는 자동으로 공식 전체 이름으로 변환되어 검색됩니다.<br>  - 예: "BFS" → "Breadth-first Search", "DP" → "Dynamic Programming"<br>  - **카테고리 확장 검색:** 상위 카테고리를 선택하면 하위 태그들도 자동으로 포함하여 검색됩니다.<br>    - "Graph Theory" → "BFS", "DFS", "Dijkstra", "Bellman–ford", "Floyd–warshall", "Topological Sorting", "MST", "LCA", "Bipartite Matching" 등 포함<br>    - "Dynamic Programming" → "Knapsack", "LCS", "LIS", "Bitmask" 등 포함<br>    - "Data Structures" → "Stack", "Queue", "Deque", "Segment Tree", "Trie" 등 포함<br>    - "Mathematics" → "Number Theory", "Primality Test", "Sieve", "Combinatorics" 등 포함<br>    - "String" → "KMP", "Trie", "Suffix Array", "Aho-corasick" 등 포함<br>    - 계층 구조에 없는 카테고리(예: "Implementation")는 자기 자신만 검색됩니다.<br>  - 미지정 시 모든 카테고리에서 추천<br>- `language` (String, optional): 문제 언어 필터<br>  - 가능한 값: "ko" (한국어), "en" (영어)<br>  - 미지정 시 모든 언어에서 추천 | `List<ProblemResponse>`<br><br>**ProblemResponse 구조:**<br>- `id` (String): 문제 ID<br>- `title` (String): 문제 제목<br>- `category` (String): 문제 카테고리<br>- `difficulty` (String): 난이도 티어명 (예: "BRONZE", "SILVER")<br>- `difficultyLevel` (Int): Solved.ac 난이도 레벨 (1-30)<br>- `url` (String): 문제 URL<br>- `language` (String): 문제 설명 언어 ("ko" 또는 "en") | JWT Token |
...
**예시 요청 (카테고리별 추천 - 상위 카테고리 확장 검색):**
```http
GET /api/v1/problems/recommend?count=10&category=Graph Theory
Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
```
*참고: "Graph Theory"를 선택하면 "BFS", "DFS", "Dijkstra", "MST", "LCA" 등 하위 태그가 포함된 문제도 함께 검색됩니다.*

**예시 요청 (카테고리별 추천 - 한글 이름):**
```http
GET /api/v1/problems/recommend?count=10&category=그래프 이론
Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
```
*참고: 한글 이름도 지원하며, "그래프 이론"은 "Graph Theory"로 변환되어 확장 검색됩니다.*
...
| PATCH | `/api/v1/retrospectives/{retrospectiveId}` | 회고 ID로 회고를 수정합니다. **보안:** JWT 토큰의 사용자가 회고의 소유자인지 검증합니다. 소유자가 아니면 403 Forbidden이 반환됩니다. | **Headers:**<br>- `Authorization: Bearer {token}` (required): JWT 토큰<br><br>**Path Variables:**<br>- `retrospectiveId` (String, required): 회고 ID<br><br>**Request Body:**<br>`RetrospectiveRequest`<br>- `content` (String, required): 회고 내용<br>  - 유효성: `@NotBlank`, `@Size(min=10, max=5000)` (10자 이상 5000자 이하)<br>  - **DB 저장 용량:** 최대 5,000자까지 저장 가능<br>- `summary` (String, required): 한 줄 요약<br>  - 유효성: `@NotBlank`, `@Size(max=200)` (200자 이하)<br>  - 필수 항목<br>- `resultType` (ProblemResult, optional): 풀이 결과 타입 (SUCCESS/FAIL/TIME_OVER)<br>  - null 허용 (선택사항)<br>- `solvedCategory` (String, optional): 사용자가 선택한 풀이 전략(알고리즘) 태그<br>  - 유효성: `@Size(max=50)` (50자 이하)<br>  - null 허용 (선택사항)<br>- `solveTime` (String, optional): 풀이 소요 시간<br>  - 유효성: `@Size(max=50)` (50자 이하)<br>  - null 허용 (선택사항) | `RetrospectiveResponse`<br><br>**RetrospectiveResponse 구조:**<br>(위와 동일) | JWT Token |
...
| POST | `/api/v1/students/sync` | Solved.ac API를 호출하여 사용자의 최신 Rating과 Tier 정보를 동기화합니다. JWT 토큰에서 사용자 정보를 자동으로 추출합니다. | **Headers:**<br>- `Authorization: Bearer {token}` (required): JWT 토큰 | `StudentProfileResponse`<br><br>**StudentProfileResponse 구조:**<br>- `nickname` (String): 닉네임<br>- `bojId` (String): BOJ ID<br>- `currentTier` (String): 현재 티어명 (예: "BRONZE")<br>- `currentTierLevel` (Int): 현재 티어의 Solved.ac 레벨 값<br>- `consecutiveSolveDays` (Int): 연속 풀이 일수<br>- `primaryLanguage` (PrimaryLanguage, nullable): 주 언어 | JWT Token |
