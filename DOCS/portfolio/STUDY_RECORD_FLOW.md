...
4. 저장 중 `DuplicateKeyException`이 발생하면 기존 문서를 다시 읽어 갱신을 시도한다. 이 경로가 동작하려면 배포 환경에 `(studentId, problemId)` 유니크 인덱스가 있어야 한다.
...
- 도메인에는 `(studentId, problemId)` 유니크 인덱스 선언이 있지만 현재 자동 인덱스 생성은 꺼져 있고 시작 시 초기화기는 `studentId` 단일 인덱스만 보장한다. 유니크 인덱스를 별도로 생성하지 않은 환경에서는 동시 요청의 중복 회고를 막지 못한다.
