...
class AiAnalysisControllerTest {
    ...
    fun `429 지속 발생 시 최종 응답 검증`() {
        ...
        every { aiAnalysisService.analyze(any(), any(), any(), any()) } throws BusinessException(
    ...
    @Test
    @DisplayName("errorMessage가 포함된 요청을 처리한다")
    fun `analyze with errorMessage`() {
        every { aiAnalysisService.analyze(any(), any(), any(), any()) } returns "markdown with error"
        ...
        mockMvc.perform(
            ...
            .andExpect(status().isOk)
            .andExpect(jsonPath("$.markdown").value("markdown with error"))
    }
}
